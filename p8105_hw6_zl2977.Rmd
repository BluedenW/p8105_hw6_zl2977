...
summary(step_lm)
```
From the stepwise selection we get the best model:
`bwt ~ babysex + bhead + blength + delwt + fincome + gaweeks + mheight + mrace + parity + ppwt + smoken` 
with adjusted r-squared value of 0.7173 and the p-value of overall F-test is smaller than 2.2e-16, which is statistically significant (less than 0.05).
...
The distribution of r-square is roughly close to a normal distribution. However, the distribution is a little skewed and there are some outliers. 
...
From the above table, the 95% confidence interval for R-Squared is (0.8937, 0.9273).
...
From the above table, the 95% confidence interval for log(beta0_hat * beta1_hat) is (1.9648, 2.0585).